...
export function ScriptAssetManager() {
  ...
  const upload = useMutation({
    mutationFn: () => {
      if (!file) throw new Error('请选择脚本文件')
    ...
    onSuccess: async (result) => {
      ...
      setFile(null)
      setLastResult(
        `成功：${result.item.relative_path} · 参数 ${result.item.parameter_observations} · warnings ${result.item.parse_warnings}`,
      )
      showToast(
        `脚本已导入：${result.item.parameter_observations} 个参数，${result.item.parse_warnings} 个 warning`,
        'success',
      )
    },
    onError: (error) => {
      setLastResult(error instanceof Error ? `失败：${error.message}` : '失败：脚本上传失败')
      showToast(error instanceof Error ? error.message : '脚本上传失败', 'error')
    },
  })
    ...
    <section className="rounded-lg border border-bda-border bg-bda-panel p-3">
      ...
      <div className="grid gap-2">
        <p className="rounded border border-bda-border bg-bda-bg px-2 py-2 text-[11px] leading-relaxed text-bda-muted">
          上传脚本会保存并解析参数，不会自动提交到集群。节点运行或 Start workflow 时才会按插件配置生成 Jobs。
        </p>
        <label className="grid gap-1 text-xs text-bda-muted">
          ...
          <input
            ...
            placeholder={selectedModel ? `${selectedModel.model_name}/submit.lsf` : '例如 af3/submit.lsf'}
          />
        </label>
      ...
      {lastResult ? (
        <p className={`mt-3 rounded border px-2 py-2 text-xs ${lastResult.startsWith('失败') ? 'border-bda-red/40 text-bda-red' : 'border-bda-green/40 text-bda-green'}`}>
          {lastResult}
        </p>
      ) : null}

      <div className="mt-3 space-y-2">
        {isLoading ? <p className="text-xs text-bda-muted">加载脚本列表…</p> : null}
        {!isLoading && scripts.length === 0 ? (
          <p className="rounded border border-dashed border-bda-border px-3 py-4 text-center text-xs text-bda-muted">
            还没有导入脚本。上传后会显示成功或失败状态。
          </p>
        ) : null}
